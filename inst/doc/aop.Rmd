...
As you can see, the graphs are largely similar. The challenge with the R-based graph picture is that the nodes aren't labeled in a very meaningful way, but it's easy to see what the nodes map to. For instance, if you want to see what node 387 maps to, you would could do:

```{r}
steatosis_aop@nodes$"387"$data$name
```

An easier way of doing the same thing would be to use:
